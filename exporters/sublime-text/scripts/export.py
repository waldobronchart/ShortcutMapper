...
import shmaplib
from shmaplib.intermediate import IntermediateDataExporter
...
def export_file(filepath, testmode):
    ...
    exporter = IntermediateDataExporter(filepath, "Sublime Text", version, "Global Context")
    exporter.parse()
    if not testmode:
        exporter.export()
...
def main():
    parser = argparse.ArgumentParser(description="Converts Sublime Text's intermediate json data file to the web application data format.")
